...
* concurrent Queue并行队列：同一时间可以有多个任务同时执行，在并行队列中可以有多个线程；并行队列只能保证任务一开始的顺序是它们加入队列的顺序，它们执行时都是并行的，不能保证它们的执行时间和执行结束完成时间，任务的执行由系统决定。
...
| 并行队列 | 当前线程顺序执行 | 一个或多个新线程无序执行 | 同全局队列的区别：&lt;/br&gt;并行队列有名称，在MRC下需要调用`dispatch_release`进行释放; &lt;/br&gt;`dispatch_barrier`必须使用自定义的并发队列：开发第三方框架建议使用并发队列。 |
